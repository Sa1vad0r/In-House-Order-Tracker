...
router = APIRouter(
    prefix="/orders",       # all routes here automatically start with /orders
    tags=["Orders"]         # groups them in your /docs page
)
...
async def update_order(order_id: int, order: UpdateOrder):
    ...
    if order.customer_name is not None:
        stored_order["customer_name"] = order.customer_name
